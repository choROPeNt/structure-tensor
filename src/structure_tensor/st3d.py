...
def structure_tensor_3d(
    ...
    Args:
        volume: A 3D array. Pass `numpy.ndarray` to avoid copying volume.
...
def eig_special_3d(
    ...
    s = lib.einsum("ij,ij->j", S[3:], S[3:], out=tmp[1])
    s *= 2

    # Compute p.
    p = lib.einsum("ij,ij->j", Sq, Sq, out=tmp[2])
    ...
    del s
    p_inv[:] = 0
    lib.divide(1, p, out=p_inv, where=p != 0)
    ...
    if full:
        ...
        l = lib.einsum("ijk,ijk->jk", vec, vec, out=vec_tmp)[:, lib.newaxis]
