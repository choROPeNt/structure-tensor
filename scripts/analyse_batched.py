...
def setup_logging(level: int = logging.INFO) -> logging.Logger:
    ...
    return logging.getLogger("analyse_volume")
...
def main(config_path: Path) -> None:
    ...
    logger.info("🌫️  Gaussian: r=%g px | sigma=%g | rho=%g | axes=%s", r, sigma, rho, axes)
    ...
    with H5BlockReader(in_path, keys_in, block_size=block_size, dtype=lib.float32, strict=False) as reader, \
        ...
        for (zsl, ysl, xsl), batch in iter(reader): # type: ignore
            ...
            print("block slices:", zsl, ysl, xsl)
            print("shape:", vol.shape)
            print("dtype:", vol.dtype)
            print("nbytes:", vol.nbytes / 1024**2, "MB")
            ...
            _, vec = eigh_baseline_3d(S, full=False)  # expect vec: (3, bz, by, bx)
            ...
            # # NOTE: writer currently writes by Z-slice only.
            # # For true 3D block writing, extend H5BlockWriter with write_block_3d (see below).
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=Path,  # argparse returns a Path directly ✅
        required=True,
